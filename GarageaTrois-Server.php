...
if (isset($uid) && $uid == 'nfc0' && isset($did) && $did !=''){
	...
	if (($did_exists == '0' && $did_allowed == '0') || ($did_exists == '1' && $nfc_allowed != '1')){
		...
		$stringData = 'New NFC request from ' . $_POST['DID'] . "\n";
	...
	if($adminaction == "Grant" || $adminaction == "Revok")
	{
		if (isset($nfc) && $nfc != ''){
			...
			if ($nfc == 'exclusive'){
				...
				$nfc = (($_POST['AdminAction'] == "Revok") ? '0' : '1');
				$forcenfc = (($_POST['AdminAction'] == "Revok") ? '1' : '0');
			}
		}
...
if (isset($_POST['switch'])){
	...
	$sql = 'INSERT INTO log (name, uid, did, action, date) '.
	...
	$txt = $users[$_POST['UID']] . " toggled " . $_POST['switch'] . ' @ ' . $stringData;
	...
	else if (array_key_exists($_POST['DID'], $disallowed_devices)){
		$granted = 'Denied (Device)';
		echo 'Access denied';
	}
	else if (array_key_exists($_POST['DID'], $nfc_only_devices)){
	...
	$stringData = $users[$_POST['UID']] . ' from ' . $_POST['DID'] . "\n";
